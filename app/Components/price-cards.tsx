    ...
    useEffect(() => {
        if (cryptocurrencies) {
            try{
                    ...
                    if(
                        ...
                        return {
                            ...
                            change: `${(coin.price_change_percentage_24h as number).toFixed(
                                2)}%`,
                            changeDirection: (coin.price_change_percentage_24h as number) >= 0? 
                            'up' 
                            : 'down',
                            icon: coin.id === 'bitcoin'? (
                            <BsCurrencyBitcoin className="text-orange-600 text-xl"/> )
                            : coin.id === 'ethereum'? (<FaEthereum className="text-blue-600 text-xl"/>) 
                ...
                console.error('Error occurred while processing cryptocurrency data:', error);
            }
        }
    },[cryptocurrencies]);
            ...
            {threeTopCurrencies?.map((crypto: PriceCardsData, index: number) =>(
                ...
                className='shadow-none border-none flex justify-between items-end p-4'>
                    <div className="flex gap-3">
                        <div className={`flex h-11 w-11 items-center justify-center ${crypto.bgColor} rounded-lg`}>
                            {crypto.icon}
                        </div>
